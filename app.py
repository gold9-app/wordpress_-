...
def api_generate_html():
    ...
    # 커스텀 지침이 있으면 사용, 없으면 기본 지침 사용
    system_prompt = custom_instruction if custom_instruction else CLAUDE_SYSTEM_PROMPT

    # 커스텀 지침일 때는 사용자 메시지도 조정
    if custom_instruction:
        user_content = f"다음 주제로 HTML 형식으로 글을 작성해주세요:\n\n{prompt}"
    else:
